%% Krahasimi Final - Subjekti 3
clear; clc;

% Rruga ku ndodhen sinjalet e procesuara
path = fullfile('results', 'sub3', 'signals');
...
% 5. Shfaq rezultatin në ekran
fprintf('--- REZULTATET PËR SUBJEKTIN 3 ---\n');
fprintf('Mesatarja RMS Agresive: %.4f\n', mean_aggr);
fprintf('Mesatarja RMS Normale: %.4f\n', mean_norm);
fprintf('Raporti (Agresiv/Normal): %.2f fish\n', ratio);

% 6. Krijo Grafikun Krahasues (Versioni i përmirësuar)
figure;
...
title(['Krahasimi RMS - Subjekti 3 (Raporti: ', num2str(ratio, '%.2f'), ')']);
ylabel('Vlera RMS');
grid on;

% 7. Ruaj grafikun te folderi comparisons
saveas(gcf, fullfile('results', 'sub3', 'comparisons', 'Krahasimi_Final_Sub3.png'));